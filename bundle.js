...
(0, _jquery2['default'])(function () {
    ...
    function draw(a, b, yMin, yMax) {
        ...
        var axisPen = new _utils.Pen(1, 'black');
        var gridPen = new _utils.Pen(1, 'black');
        var chartPen = new _utils.Pen(2, 'lime');
        var font = new _utils.Font(11, 'Arial');
        ...
        for (var x = a; x < b + step_x / 20; x += 0.008) {
            points.push(new _utils.Point(toX(x), toY(Math.tan(x) * Math.cos(x + 30))));
        }
    ...
    var mouseDown = _rx.Observable.fromEvent(canvas, 'mousedown');
    var mouseMove = _rx.Observable.fromEvent(canvas, 'mousemove');
    var mouseUp = _rx.Observable.fromEvent(canvas, 'mouseup');

    var font = new _utils.Font(10, 'Arial');
    var pen = new _utils.Pen(1, 'black');

    mouseDown.flatMap(function (e) {
        var firstPoint = new _utils.Point(e.pageX, e.pageY);

        context.drawString('' + Xto(e.pageX).toFixed(2) + ' : ' + Yto(e.pageY).toFixed(2), font, 'navy', e.pageX + 8, e.pageY - 8);

        return mouseMove.filter(function (e) {
            return e.button === 0;
        })['do'](function (e) {
            draw(a, b, yMin, yMax);
        ...
        }).merge(mouseMove.filter(function (e) {
            return e.button === 2;
        })['do'](function (e) {
            ...
            firstPoint = new _utils.Point(e.pageX, e.pageY);

            context.drawString('' + Xto(e.pageX).toFixed(2) + ' : ' + Yto(e.pageY).toFixed(2), font, 'navy', e.pageX + 8, e.pageY - 8);
            draw(a, b, yMin, yMax);
        })).merge(mouseUp.filter(function (e) {
            return e.button === 0;
        })['do'](function (e) {
            ...
            yMin = yMin + Yto(firstPoint.y) - Yto(e.pageY);
            yMax = yMax + Yto(firstPoint.y) - Yto(e.pageY);
        }))).takeUntil(mouseUp['do'](function (e) {
            draw(a, b, yMin, yMax);
        }));
    }).merge(mouseDown).subscribe(function (e) {
        switch (e.type) {
            case 'mouseup':
                {
                    canvas.css({
...
},{"./canvas":2,"./utils":3,"jquery":4,"rx":5}],2:[function(require,module,exports){
'use strict';
